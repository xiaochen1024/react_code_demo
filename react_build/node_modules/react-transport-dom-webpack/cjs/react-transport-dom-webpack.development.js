    ...
    case 'M':
      {
        ...
        resolveModule(response, _id, _json);
        return;
      }

    case 'E':
      {
        var _colon2 = row.indexOf(':', 1);

        var _id2 = parseInt(row.substring(1, _colon2), 16);

        var _json2 = row.substring(_colon2 + 1);

        var errorInfo = JSON.parse(_json2);
        resolveError(response, _id2, errorInfo.message, errorInfo.stack);
        return;
      }
